---
course: "[[300 Computing/Software Engineering/COMP3500 - Software Engineering|Software Engineering]]"
type: lecture
date: 2025-02-10
week: 1
session: 1
instructor: 
tags:
  - lecture
  - software-engineering
  - agile
  - scrum
status: in-progress
---

# Week 01-02 - Agile

> [!info] Lecture Details
> **Course:** [[300 Computing/Software Engineering/COMP3500 - Software Engineering|Software Engineering]]
> **Type:** Lecture
> **Date:** Monday, 10 February 2025
> **Week:** 1-2
> **Lecture #:** 1
> **Instructor:** 
> **Recording:** [Link]()

---

## Overview

Introduction to Agile software development methodology covering estimating effort, process decisions, milestones, the Agile Manifesto, incremental development, Scrum framework, user stories, sprints, test automation, continuous integration, personas, and scenarios.

---

## Key Concepts

### 1. Agile Fundamentals

- Estimating Effort - How long is going to take to write a software product for project planning.
- Process Decisions
  - Tracking Bugs
  - Version Control (Git)
  - Write down requirements for a project
  - Testing
  - Status meetings
- Milestones
  - For tracking progress
  - It's common to take longer for tasks

Agile works by allowing you to make changes to a project quickly. There are lots of methods, there is no best method.

- Before Agile, you had Plan-Driven development that would exist for systems that would be around for a long time.
  - E.G., Westpac, running using some old Software in Melbourne.
  - E.G., some NZ company developing banking software, grew fast, transactions didn't go through on debit cards.
  - This is why people switched to Agile methods.
- Agile Manifesto: Finds better ways of developing software in the 90's
- Agile worked based all of there ideas around incremental development
  - Small steps, easy to change
  - A focus on features
  - Important features are prioritised and done first
  - There is a feedback loop for what to do

### 2. Scrum

- Product Backlog (features)
  - Product: The software product being developed by your team
  - Product Owner: A team member who is responsible for identifying product features. They review work done and help to test the product.
- Product Backlog
  - A to-do list of items such as bugs, features and product improvements that the Scrum team have not yet completed.
  - Product Backlog Items (PBIS) 
    - Work In Progress
    - Awaiting
    - Done
- 2-4 weeks to implement a feature from backlog
  - 24 hour daily scrum meetings
  - Development team: A small self-organising team of 5-8 people who are responsible for developing the product. 
- Potentially shippable product

#### Scrum Terminology
- Sprint: A short period (2-4 weeks), when a product is incrementally developed. 
- Scrum: A daily team meeting where progress is reviewed and work to be done that day.
- ScrumMaster: A team coach who guides the team in the effective use of Scrum.
  - Not a project manager, but a coach ("You follow this process")
- Velocity: An estimate of how much work a team can do in a single sprint. 
- Product Owner: Responsible for ensuring that the development team are always focused on the product they are building.
- Self-organising Teams: Make their own decisions and work by discussing issues and making decisions by consensus.

### 3. User Stories

- Specifications
  - Small: Should be able to fit on a card.
  - Concise: A list of requirements, simple, easy to understand.
    - Doesn't need all of the details
    - It's an idea
- Template:
  - As a [Role] - Who wants it?
  - I want [Function] -What is it
  - So that [Value] - Why I want it
- Epic Story: A user story that is too complicated, so it gets split up into multiple user stories.
- Testing: Should be testing available to understand when the test is done.
- E.G., As a user, I want a location map of all available scooters, so I know where the available scooters are.

#### Evaluating User Stories
- Independent
- Negotiable
  - Details can be changed, a user story should capture the idea, not the details.
- Valuable
  - Should have some utility for the consumer
- Estimable
  - Helps keep the size small
- Small
  - An idea that's two persons weeks of work
- Testable
  - When is it done. Need to test it to know when it's done.

### 4. Sprints

- Timeboxed Sprints
  - Duration of 1-4 weeks, development stops at the end of a sprint whether or not the work has been completed.
    - Then you think again, improve, etc.
  - During a sprint, the team works on the items from the product backlog.
  - Products are delivered in a series of sprints, each sprint delivers an increment to a product.
- Sprint Planning: Work out what's going to happen during the Sprint
- Sprint Execution: The team works to implement the Sprint backlog items that has been chosen.
- Sprint Reviewing: The work done in the sprint is reviewed by the team, and they reflect on what went well and what didn't. 
- Scrum: Review the Sprint, Sprint backlog, what's going on.

#### Agile Activities
- Test automation: As far as possible, product testing should be automated.
- Continuous Integration: As soon as changes are made, you try and integrate it with the app, good for testing and seeing if anything breaks.

### 5. Personas

- User Understanding: Understand the potential users and customers.
  - You can interview them, or use software to see what they're doing.
  - Put yourself in the mindset of the user
  - Develop stories from the users perspective
- Feature Description
  - What is the feature
  - What is the input, what is the output etc.

### 6. Scenarios

- A scenario on how users or a group of users will use your system.
- You learn user stories from these scenarios.
- Describes a situation where a user or group of users interacts with the system or product. It helps visualize how people will use the features and functions of the software.

- Feature Creep: When features are added without thinking about the feature. Does it add anything new?

---

## Notes

---

## Examples

> [!example] User Story Template
> As a user, I want a location map of all available scooters, so I know where the available scooters are.

> [!example] Plan-Driven vs Agile
> Plan-Driven development was used for systems that would be around for a long time (e.g., Westpac, banking software). However, when systems needed to grow fast, plan-driven approaches failed (e.g., transactions didn't go through on debit cards), leading to the adoption of Agile methods.

---

## Formulas & Definitions

| Term | Definition |
|------|------------|
| Agile | Software development methodology allowing quick changes through incremental development |
| Plan-Driven Development | Traditional development with detailed upfront planning and rigid steps |
| Product Backlog | A to-do list of items (bugs, features, improvements) not yet completed by the Scrum team |
| Product Owner | Team member responsible for identifying product features and reviewing work |
| Sprint | A short period (1-4 weeks) when a product is incrementally developed |
| Scrum | Daily team meeting where progress is reviewed and work for the day is planned |
| ScrumMaster | Team coach who guides the team in effective use of Scrum practices |
| Velocity | Estimate of how much work a team can do in a single sprint |
| User Story | Specification template: "As a [Role], I want [Function], so that [Value]" |
| Epic Story | A user story that is too complicated and gets split into multiple user stories |
| Continuous Integration | Practice of frequently integrating code changes and testing them |
| Persona | Fictional character representing a user type to help design features |
| Scenario | Description of how users interact with the system to reveal requirements |
| Feature Creep | Adding features without considering if they add value |

---

## Questions & Discussion

- [ ] What is the purpose of estimating effort in Agile?
- [ ] Name two process decisions commonly made in Agile development.
- [ ] What is a milestone in Agile, and why is it useful?
- [ ] How does Agile differ from Plan-Driven development?
- [ ] What is the Agile Manifesto, and why was it created?
- [ ] Explain the concept of "incremental development" in Agile.
- [ ] What is a product backlog?
- [ ] Who is responsible for prioritising the product backlog items?
- [ ] Describe what happens in a Scrum meeting.
- [ ] What is the role of a ScrumMaster in a Scrum team?
- [ ] Define a sprint in Agile.
- [ ] What is "velocity," and how is it used in Agile?
- [ ] Explain the structure of a user story using the "As a [Role], I want [Function], so that [Value]" format.
- [ ] What is an "epic" in Agile?
- [ ] List three criteria that make a user story effective.
- [ ] What is the purpose of a sprint review?
- [ ] Describe what happens during sprint planning.
- [ ] What is test automation, and why is it important in Agile?
- [ ] Explain "continuous integration" in Agile.
- [ ] What is a persona, and how does it help in Agile development?
- [ ] Give an example of "feature creep" in software development.

---

## Action Items

- [ ] Review lecture recording
- [ ] Complete practice questions
- [ ] Understand Agile vs Plan-Driven development
- [ ] Practice writing user stories
- [ ] Learn about Scrum framework and terminology

---

## Related Notes

- [[300 Computing/Software Engineering/COMP3500 - Software Engineering|Course Summary]]

---

## References

- Lecture materials
