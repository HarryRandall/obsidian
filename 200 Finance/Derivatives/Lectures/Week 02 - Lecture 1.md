...
Hedging strategies using futures covering interest rate compounding, short selling, forward and futures pricing, hedging principles, and hedging using index futures.
...
- Continuous formula: $FV = PVe^{rT}$

**Present Value**
- Original formula: $PV = \frac {FV} {(1+r)^T}$
- Continuous formula: $PV = FVe^{-rT}$
...
- Only available for some investment assets, most often stocks
...
- You may profit from storage fees (if you sell the asset, you don't have to store it, so there is money there for you).
...
### 4. The Price of Forward and Futures
...
In general if:
- $F_0 > S_0e^{rT}$ arbitrageurs can make a riskless profit from **BUYING** the asset and entering into a short forward contract on this asset. This strategy is financed by borrowing funds at the risk free rate.
- $F_0 < S_0e^{rT}$ arbitrageurs can make a riskless profit from **SHORTING** the asset and entering into a long forward contract on this asset. This strategy is financed by borrowing funds at the risk free rate.
...
- Consider a forward contract on an asset that will provide a perfectly predictable cash income to the holder
  ...
  - Coupon bonds
  - D is the present value of this income
  $$F_0 = (S_0 - PV(D))e^{rT}$$

This is saying you buy the stock at $S_0$ but receive some income whilst holding it, so you need to deduct $PV(D)$ to find the price.

> [!example] Known Income Example
...
- Income on asset is often expressed as yield rather than dollar value
  ...
  - $d$ as the average yield per annum on an asset during the life of a forward contract with continuous compounding
  $$F_0 = S_0e^{(r-d)T}$$

> [!example] Known Yield Example
...
> [!example] Stock Index Futures Example
...
- $F_0$ forward/futures price in dollars of one unit of foreign currency
  $$F_0 = S_0e^{(r-r_{foreign})T}$$
...
- **Consumption assets**
  - Held primarily for consumption
...
- Next, we come to today, what's the value of our contract?
  ...
  - $F_1 = S_1e^{r_1T_1}$ is the current forward price
...
> [!example] Value of Forward Example
...
- **Basis risk**
  ...
  - In perfect hedging, basis risk = 0
...
- **Cross hedging**
  ...
  - Examples
    ...
    - There are no future contracts on aviation fuel
...
- **Hedge ratio** is the ratio of the size of the position taken in the futures contract to the size of the asset with exposure
  $$Hedge Ratio = \frac {size of futures} {size of underlying asset}$$
  $$= \frac {number of contracts (N) \times size per contract (A)} {size of underlying asset (P)}$$
  $$h = \frac {N \times A} {P}$$
  $$N = h \times \frac {P}{A}$$
...
> [!example] Optimal Hedge Ratio Examples
...
> [!example] Qantas Hedging Example
...
>   - The std of the price in heating oil is $\sigma_F = 3.13\%$
...
### 12. Hedging Using Index Futures
...
- Use stock index to hedge a stock portfolio
- The optimal hedge ratio is the beta of the portfolio to be hedged
  $$h = \beta$$
- The number of index futures contracts to be shorted
  $$N = \beta\frac{P}{A}$$
...
> [!example] Index Futures Hedging Example
...
| Term | Definition |
|------|------------|
| Continuous Compounding | Interest compounded at every instant, using $e^{rT}$ |
| $R_c$ | Continuously compounded interest rate |
| $R_m$ | Nominal interest rate compounded periodically |
| Short Selling | Selling an asset that is not owned, borrowing from broker |
| Forward Price (No Income) | $F_0 = S_0e^{rT}$ |
| Forward Price (Known Income) | $F_0 = (S_0 - PV(D))e^{rT}$ |
| Forward Price (Known Yield) | $F_0 = S_0e^{(r-d)T}$ |
| Forward Price (Currency) | $F_0 = S_0e^{(r-r_{foreign})T}$ |
...
| Hedge Ratio | $h = \frac{N \times A}{P}$ |
| Optimal Hedge Ratio | $h = p \frac{\sigma_S}{\sigma_F}$ |
| Index Futures Hedge | $N = \beta\frac{P}{A}$ |
| Basis | Spot price of asset to be hedged - futures price of contract used |
| Basis Risk | Risk associated with imperfect hedging |
| Cross Hedging | Using futures contract on correlated asset when exact contract doesn't exist |
...
- [ ] How do you calculate the optimal hedge ratio?
...
- [ ] Practice forward/futures pricing calculations
