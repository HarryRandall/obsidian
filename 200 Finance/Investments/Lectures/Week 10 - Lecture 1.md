...
Term structure of interest rates covering yield curves, spot rates, forward rates, theories of term structure, and applications including bond pricing and recession forecasting.
...
- Term structure in the base case - risk free rate
	- Yield on zero coupon government bonds with different maturities
	- Zeros = zero coupon government bonds
...
- Suppose a newly issued 2-year government bond
- Face value F = $100
	- (1) Zero coupon
...
$$
f_{2,5} = \left[\frac{(1 + r_{0,5})^5}{(1 + r_{0,2})^2}\right]^{\frac{1}{5 - 2}} - 1 = 3.10\%
$$
...
- Procedure
	- Borrow a loan of $\frac{5000}{(1+r_{0,2})^2} = \$4,796.435$ for 5 years at $r_{0,5}$
...
> [!example] Bond Pricing with Term Structure
> A 3-year coupon bond with 3% coupon rate, face value $100, priced using spot rates: 2% (1yr), 3% (2yr), 4% (3yr) yields price of $97.34.

> [!example] Forward Rate Calculation
> To borrow $5,000 in 2 years for 3 years, forward rate $f_{2,5} = 3.10\%$ can be calculated from spot rates using the formula $f_{t,T} = \left[\frac{(1 + r_{0,T})^T}{(1 + r_{0,t})^t}\right]^{\frac{1}{T - t}} - 1$
...
| Spot Rate | Annualised interest rate from today (time 0) to future date T |
| Forward Rate | Annualised interest rate for period from future date t to future date T |
| Zero Coupon Bond | Bond that pays no coupons, only face value at maturity |
| Forward Rate Formula | $f_{t,T} = \left[\frac{(1 + r_{0,T})^T}{(1 + r_{0,t})^t}\right]^{\frac{1}{T - t}} - 1$ |
| Expectations Hypothesis | Forward rate equals expected future spot rate |
| Inverted Yield Curve | Downward sloping yield curve, often signals recession |
...
- [ ] How is the term structure used in bond pricing?
...
- [ ] Practice calculating forward rates from spot rates
- [ ] Understand bond pricing with term structure
- [ ] Review theories of term structure
...
- [[200 Finance/Investments/Lectures/Week 07 - Lecture 1|Week 07 - Lecture 1]]
