...
Interest rate risk covering bond price sensitivity to yield changes, duration (Macaulay's and modified), convexity, and bond portfolio management including immunization strategies.
...
$$
\frac{\frac{dP_0}{dy}}{P_0} = -\frac{1}{(1+y)} \cdot \frac{1}{P_0}\left[\sum_{t=1}^{T} t \times \frac{CF_t}{(1+y)^t}\right]
$$
...
$$
\frac{\frac{dP_0}{dy}}{P_0} = -MD = -\frac{D}{(1+y)}
$$
...
$$
P = \frac{8}{(1 + 9\%)} + \frac{8}{(1 + 9\%)^2} + \frac{100 + 8}{(1 + 9\%)^3} = 97.4687
$$

- Step 2: The duration and modified duration

$$
D = \frac{1}{P}\left[\sum_{t=1}^{T} t \times \frac{CF_t}{(1 + y)^t}\right] = \frac{1}{97.4687}\left[\frac{8}{(1 + 9\%)} + 2 \times \frac{8}{(1 + 9\%)^2} + 3 \times \frac{100 + 8}{(1 + 9\%)^3}\right] = 2.7803
$$

$$
MD = \frac{D}{(1 + y)} = \frac{2.78}{(1 + 9\%)} = 2.5507
$$
...
$$
\Delta P = -MD \times \Delta y \times P
$$

- Including convexity adjustment

$$
\frac{\Delta P}{P} = -MD \times \Delta y + \frac{1}{2} \times \text{Convexity} \times (\Delta y)^2
$$
...
> A 3-year 8% coupon bond with 9% yield has Macaulay duration of 2.78 years and modified duration of 2.55. A 0.1% yield increase causes approximately 0.255% price drop.

> [!example] Immunization
> An insurance company with $10M liability in 3 years can immunize by buying 173,160 units of 2-year bond A and shorting 84,539 units of 1-year bond B to match liability duration.
...
| Term | Definition |
|------|------------|
| Macaulay's Duration | $D = \frac{1}{P_0} \sum_{t=1}^{T} t \times \frac{CF_t}{(1+y)^t}$ - weighted average time to receive cash flows |
| Modified Duration | $MD = \frac{D}{(1+y)}$ - sensitivity measure for price changes |
| Duration Approximation | $\frac{\Delta P}{P} = -MD \times \Delta y$ |
| Convexity | $\frac{1}{P(1+y)^2} \sum_{t=1}^{T} \frac{CF_t}{(1+y)^t} (t^2 + t)$ - second-order price sensitivity |
| Convexity Adjustment | $\Delta P = -MD \times \Delta y \times P + \frac{1}{2} \times \text{Convexity} \times (\Delta y)^2 \times P$ |
| Immunization | Strategy to match asset and liability durations to hedge interest rate risk |
| Zero-Coupon Duration | Duration equals maturity for zero-coupon bonds |

---

## Questions & Discussion

- [ ] How does duration measure bond price sensitivity to yield changes?
- [ ] What is the difference between Macaulay's duration and modified duration?
- [ ] When is convexity adjustment important?
- [ ] How does immunization work to hedge interest rate risk?
...
- [ ] Practice calculating duration and modified duration
- [ ] Understand immunization strategies
- [ ] Practice convexity calculations
