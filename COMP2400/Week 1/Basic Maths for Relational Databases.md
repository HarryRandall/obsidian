Lecture - [link](https://wattlecourses.anu.edu.au/pluginfile.php/3887327/mod_resource/content/11/w01l02_db_math_concepts.mp4) 

---
#### Set
- A container of things, that we want to put different objects into.
...
2. Each element cannot be in the set more than once;
	1. $\{Monday, Tuesday, Wednesday, Friday, Friday\}$ is not a set
	2. $\{Monday, Tuesday, Wednesday, Friday\}$ is a set
	3. Note that Multisets allow to have duplicate elements.
Two ways of specifying a set
1. $\{x_1, ..., x_n\}$ (i.e., list all elements in a set)
	1. $\{1,2,3,4,5\}$
	2. $\{Sydney, Melbourne, Canberra\}$
	3. $\{\}$ or $∅$, i.e., the empty set.
2. $\{x | y\}$ (i.e., describes the elements in the set $x$ that satisfy the conditions of $y$).
	1. $\{x |$ student is enrolled in comp2400 $\}$ 
	2. $\{x |$ $x$ is an integer and $x > 0$ $\}$ 
...
1. A set that contains $\{2\} \in A$ 
	1. This is incorrect as $\{2\}$ is a **proper subset** of $A$.
2. $true \subset B$ 
	1. This is incorrect as $true \in B$ and $\{true\} \subset B$ 
3. $\{2,3\} \subseteq \{A\} \cup \{B\}$
	1. This means - $\{1,2,3,true,false\} \subseteq \{2,3\}$
	2. This is true as the set $\{2,3\}$ falls inside the larger subset.
4. $2 \in \{A\} \cap \{B\}$
	1. This is false as there are no elements that intersect the set $\{A\}$ nor $\{B\}$
5. $2 \in \{A\} - \{1,3,5\}$
	1. This is true because $\{A\} - \{1,3,5\}$ results in the set $\{2,3\}$ and $2$ is an element of that set.
6. $\{1, 4\} \subseteq \{A\} - \{B\}$
	1. This is false as $4$ is not a subset of neither $\{A\}$ nor $\{B\}$.
7. $∅ \cap \{B\} = ∅$ 
	1. This is true, as $\cap$ is the union (the intersection) between the empty set and $\{B\}$ and there are no items in the empty set so no intersection.
